function add_solver_xml!(::NEOSSolver{:Xpress, :MPS}, m::NEOSMathProgModel)
...
function parse_status!(::NEOSSolver{:Xpress, :MPS}, m::NEOSMathProgModel)
...
function parse_objective!(::NEOSSolver{:Xpress, :MPS}, m::NEOSMathProgModel)
	m.objVal = parse(Float64, match(r"Objective function value is\W+?(-?[\d.]+)", m.last_results).captures[1])
end

function parse_solution!(::NEOSSolver{:Xpress, :MPS}, m::NEOSMathProgModel)
