function add_solver_xml!(::NEOSSolver{:MOSEK, :MPS}, m::NEOSMathProgModel)
...
function parse_status!(::NEOSSolver{:MOSEK, :MPS}, m::NEOSMathProgModel)
...
function parse_objective!(::NEOSSolver{:MOSEK, :MPS}, m::NEOSMathProgModel)
	sci = match(r"PRIMAL\W+?OBJECTIVE\W+?:\W+?(-?[\d\.]+e[\+\-]\d+)", m.last_results).captures
	m.objVal = parse(Float64, sci[1])
end

function parse_solution!(::NEOSSolver{:MOSEK, :MPS}, m::NEOSMathProgModel)
...
function parse_duals!(::NEOSSolver{:MOSEK, :MPS}, m::NEOSMathProgModel)
